...
import { Edit, PlusCircle, CheckCircle, XCircle, Trash2 } from "lucide-react";
...
const hasNotes = (subject: Subject) => {
    ...
    return modulesArray.some(module => {
        if (!module || !module.notes) return false;
...
export default function AdminDashboard() {
  ...
  const [activeAccordion, setActiveAccordion] = useState<string>("cse");

  useEffect(() => {
    const hash = window.location.hash.substring(1);
    if (hash && branches[hash]) {
      setActiveAccordion(hash);
    }
  }, [branches]);

  const subjectsByBranch = useMemo(() => {
    return Object.keys(branches).reduce((acc, branchId) => {
      acc[branchId] = subjects.filter(s => s.branch?.includes(branchId));
      return acc;
    ...
    <div className="space-y-8">
      ...
      >
        {Object.entries(branches).map(([branchId, branchInfo]) => {
          const branchSubjects = subjectsByBranch[branchId] || [];
          const subjectsBySemester = branchSubjects.reduce((acc, subject) => {
          ...
          return (
            <AccordionItem key={branchId} value={branchId} id={branchId} className="border rounded-xl scroll-mt-20">
